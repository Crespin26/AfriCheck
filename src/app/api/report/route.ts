...
export async function POST(request: Request) {
  ...
  const error = (message: string, status: number) =>
    Response.json(
      { error: message, code: "INVALID_REPORT", requestId },
      { status, headers },
    );
  if (!decision.allowed)
  ...
  try {
    ...
    try {
      input = parseReportInput(parsed);
    } catch {
      return error("Les données du rapport sont invalides.", 400);
    }
    let pdf: Uint8Array;
    try {
      pdf = await generateReportPdf(input);
    } catch {
      return Response.json(
        {
          error: "Le rapport ne peut pas être généré pour le moment.",
    ...
    const host = displayHostname(input.finalUrl)
      .replace(/[^a-z0-9.-]+/gi, "-")
      .slice(0, 80);
    return new Response(Buffer.from(pdf), {
      ...
      headers: {
        ...
        "content-disposition": `attachment; filename="africheck-${host}-${date}.pdf"`,
