...
vi.mock("@/lib/observability", () => ({
  createRequestId: () => "report-request",
}));
  ...
  findings: [
    {
      ...
      points: 10,
      maxPoints: 10,
      observation: "Connexion chiffree.",
...
describe("POST /api/report", () => {
  it("retourne un PDF avec un nom de fichier neutralisé", async () => {
    ...
    expect(response.headers.get("content-disposition")).toContain(
      "africheck-example.com-2026-08-27.pdf",
    );
  ...
  it("rejette les données invalides", async () => {
    ...
    const response = await POST(
      new Request("https://app.test/api/report", {
        method: "POST",
        body: JSON.stringify({ ...payload, score: 999 }),
      }),
    );
  ...
  it("distingue une panne interne de génération", async () => {
    vi.mocked(generateReportPdf).mockRejectedValueOnce(
      new Error("internal path"),
    );
    const { POST } = await import("./route");
    const response = await POST(
      new Request("https://app.test/api/report", {
