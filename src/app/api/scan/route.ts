...
export async function POST(request: Request) {
  ...
  if (!decision.allowed) {
    ...
    return errorResponse(
      "Trop de diagnostics demandés. Réessayez plus tard.",
      "RATE_LIMITED",
      429,
    );
  }
  ...
  try {
    ...
    if (typeof body.url !== "string")
      return reject("Une adresse web est requise.", 400);

    let verifiedHostname: string | undefined;
    if (body.ownership !== undefined) {
      if (!body.ownership || typeof body.ownership !== "object")
        return reject("La preuve de domaine est invalide.", 400);
      const ownership = body.ownership as {
        proof?: unknown;
        clientSecret?: unknown;
      };
      if (
        typeof ownership.proof !== "string" ||
        typeof ownership.clientSecret !== "string"
      )
        return reject("La preuve de domaine est invalide.", 400);
      verifiedHostname = verifyOwnershipProof(
        ownership.proof,
        ownership.clientSecret,
        domainVerificationKey(),
      ).hostname;
      if (normalizeUrl(body.url).hostname !== verifiedHostname)
        throw new DomainVerificationError("DOMAIN_MISMATCH");
    }
    ...
    if (!release) {
      ...
      return Response.json(
        {
          error:
            "Le scanner est temporairement occupé. Réessayez dans quelques secondes.",
          code: "SCAN_BUSY",
          requestId,
        },
    ...
    try {
      result = await scanWebsite(body.url);
    } finally {
      release();
    }
    let history:
      ...
      | undefined;
    if (verifiedHostname) {
      if (new URL(result.finalUrl).hostname !== verifiedHostname)
        history = { saved: false, code: "DOMAIN_MISMATCH" };
      else {
        try {
          history = {
            saved: true,
            id: await saveScanHistory(verifiedHostname, result),
          };
        } catch (historyError) {
          history = {
            saved: false,
            code:
              historyError instanceof DatabaseConfigurationError &&
              historyError.code === "DATABASE_DISABLED"
                ? "HISTORY_DISABLED"
                : "HISTORY_UNAVAILABLE",
          };
        }
