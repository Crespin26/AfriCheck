...
describe("readiness", () => {
  ...
  it("borne la concurrence des scans", () => {
    expect(readiness({ SCAN_MAX_CONCURRENCY: "8" }, "24.0.0")).toMatchObject({ ready: true, checks: { configuration: "ok" } });
    expect(readiness({ SCAN_MAX_CONCURRENCY: "0" }, "24.0.0")).toMatchObject({ ready: false, checks: { configuration: "error" } });
    expect(readiness({ SCAN_MAX_CONCURRENCY: "33" }, "24.0.0")).toMatchObject({ ready: false, checks: { configuration: "error" } });
  });
